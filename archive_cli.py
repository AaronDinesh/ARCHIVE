...
from tools.rclone_io import ensure_path, list_folders, move_local_to_remote
...
def route(
    ...
    auto: bool = typer.Option(
        False, "--auto", help="Automatically move to a chosen or top folder."
    ),
    chosen: str = typer.Option(
        None, "--chosen", help="Explicit folder path to use (overrides auto selection)."
    ),
    allow_create: bool = typer.Option(
        False, "--allow-create", help="Create missing folders if needed."
    ),
    ...
    if len(text.strip()) < 30:
        print(
            "[yellow]Warning:[/yellow] very little text extracted (scan? quality low). Proceeding anyway."
        )
...
#             return
        ...
        if cands:
            table = Table(
                title="Top Folders", show_header=True, header_style="bold", box=box.SIMPLE_HEAVY
            )
